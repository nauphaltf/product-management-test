        ...
        <div class="col-md-6">
            <form method="post" action="{{ route('product.store') }}" id="product-form">
                ...
                <div class="mb-3">
                  ...
                  <input type="text" required class="form-control" id="product-title" name="title" value="{{ $product->title }}">
                </div>
                <div class="mb-3">
                    <label class="form-label">Product Description</label>
                    <textarea required class="form-control" id="product-description" name="description">{{ $product->description }}</textarea>
                </div>
                <div class="mb-3">
                    <label class="form-label">Product Attributes</label>
                    @foreach ($attributes as $attribute)
                    <div>{{ $attribute->title }} : 
                        <select class="form-control" name="attribute-{{$attribute->id}}" id="attribute-{{$attribute->id}}">
            ...
            <form action="{{ route('product.image-upload') }}" method="post" enctype="multipart/form-data" id="image-upload-form">
                ...
                <div class="mb-3">
                    ...
                    <img id="demo-image" src="{{ $product->main_image }}" height="100" width="100" />
                </div>
            </form>
            <div class="mb-3">
